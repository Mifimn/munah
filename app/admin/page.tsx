...
import { 
  ...
  Edit3, Trash2, X, ShieldCheck, TrendingUp, AlertCircle, Menu, FileText, Truck
} from "lucide-react";
...
export default function AdminCommandCenter() {
  ...
  const [inventory, setInventory] = useState([
    { id: "BOT-001", name: "Immunity Elixir", category: "Extracts", price: 45000, stock: 24, status: "Healthy" },
    { id: "BOT-002", name: "Sahur Gruel", category: "Nourishment", price: 15000, stock: 5, status: "Low Stock" },
    { id: "BOT-003", name: "Digestive Bitters", category: "Tinctures", price: 22000, stock: 0, status: "Depleted" },
  ]);
    ...
    <main className="min-h-screen bg-earth-silk flex flex-col lg:flex-row">
      ...
      <div className="lg:hidden bg-botanical-green w-full sticky top-0 z-20 shadow-xl">
        <div className="flex justify-between items-center p-4 border-b border-clinical-white/10">
          ...
          <button className="p-2 text-clinical-white"><Settings size={18} /></button>
        </div>
      ...
      {/* --- 3. PRODUCT MANAGER DRAWER (Responsive) --- */}
      <AnimatePresence>
        {isDrawerOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsDrawerOpen(false)} className="fixed inset-0 bg-botanical-green/40 backdrop-blur-sm z-[100]" />
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed right-0 top-0 h-full w-full max-w-[500px] bg-clinical-white z-[110] shadow-2xl flex flex-col">
              <div className="p-6 md:p-8 border-b border-botanical-green/10 flex justify-between items-center bg-earth-silk/50">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.3em] text-botanical-green/40 font-bold">Registry Form</p>
                  <h2 className="font-serif text-xl md:text-2xl text-botanical-green">Add Botanical Extract</h2>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-8">
                <form className="space-y-6">
                  <div>
                    ...
                    <input type="text" placeholder="e.g. Ancestral Root Tonic" className="w-full mt-2 border-b border-botanical-green/20 py-3 outline-none focus:border-botanical-green bg-transparent text-sm font-serif text-botanical-green" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      <label className="text-[10px] uppercase tracking-widest text-botanical-green/60 font-bold">Category</label>
                      <select className="w-full mt-2 border-b border-botanical-green/20 py-3 outline-none focus:border-botanical-green bg-transparent text-sm text-botanical-green appearance-none">
                        <option>Tinctures</option><option>Extracts</option><option>Nourishment</option>
                      </select>
                    </div>
                    <div>
                      <label className="text-[10px] uppercase tracking-widest text-botanical-green/60 font-bold">Unit Price (₦)</label>
                      <input type="number" placeholder="0.00" className="w-full mt-2 border-b border-botanical-green/20 py-3 outline-none focus:border-botanical-green bg-transparent text-sm text-botanical-green" />
                    </div>
                  </div>
                  <div>
                    <label className="text-[10px] uppercase tracking-widest text-botanical-green/60 font-bold">Initial Stock Level</label>
                    <input type="number" placeholder="Enter quantity" className="w-full mt-2 border-b border-botanical-green/20 py-3 outline-none focus:border-botanical-green bg-transparent text-sm text-botanical-green" />
                  </div>
                  <div>
                    <label className="text-[10px] uppercase tracking-widest text-botanical-green/60 font-bold">Clinical Description</label>
                    <textarea rows={4} placeholder="Detail the medical benefits..." className="w-full mt-2 border border-botanical-green/20 p-4 outline-none focus:border-botanical-green bg-transparent text-sm text-botanical-green resize-none rounded-sm" />
                  </div>
                </form>
              </div>

              <div className="p-6 md:p-8 bg-earth-silk border-t border-botanical-green/10">
                <button className="w-full bg-botanical-green text-clinical-white py-4 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] shadow-xl hover:bg-botanical-green/90 transition-all">Save to Inventory</button>
              </div>
            </motion.div>
          </>
        )}
      ...
      <div className="flex-1 lg:ml-[280px] p-4 sm:p-6 md:p-12 w-full overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-12">
          <div className="bg-clinical-white p-5 md:p-6 border border-botanical-green/5 shadow-sm rounded-sm"><div className="flex justify-between items-start mb-4"><p className="text-[10px] uppercase tracking-widest text-botanical-green/50 font-bold">Active Formulas</p><Boxes size={16} className="text-botanical-green/30" /></div><h3 className="font-serif text-2xl md:text-3xl text-botanical-green">34</h3></div>
          <div className="bg-clinical-white p-5 md:p-6 border border-botanical-green/5 shadow-sm rounded-sm"><div className="flex justify-between items-start mb-4"><p className="text-[10px] uppercase tracking-widest text-botanical-green/50 font-bold">Depleted Stock</p><AlertCircle size={16} className="text-red-800/40" /></div><h3 className="font-serif text-2xl md:text-3xl text-botanical-green">1</h3></div>
          <div className="bg-botanical-green p-5 md:p-6 border border-botanical-green/5 shadow-xl rounded-sm text-clinical-white"><div className="flex justify-between items-start mb-4"><p className="text-[10px] uppercase tracking-widest text-clinical-white/60 font-bold">Gross Value</p><TrendingUp size={16} className="text-clinical-white/40" /></div><h3 className="font-serif text-2xl md:text-3xl">₦1,240,000</h3></div>
        </div>

        {/* --- INVENTORY TABLE (Scrollable on Mobile) --- */}
        <div className="bg-clinical-white border border-botanical-green/5 shadow-sm rounded-sm overflow-hidden w-full">
          <div className="p-4 md:p-6 border-b border-botanical-green/5 bg-earth-silk/30">
            <div className="flex items-center gap-3 w-full sm:max-w-sm bg-clinical-white px-4 py-2 border border-botanical-green/10 rounded-full">
              ...
              <input type="text" placeholder="Search registry..." className="w-full bg-transparent outline-none text-xs text-botanical-green" />
            </div>
          </div>
              ...
              <tbody>
                {inventory.map((item, index) => (
                  <tr key={index} className="border-b border-botanical-green/5 hover:bg-earth-silk/20 transition-colors group">
                    <td className="p-4 md:p-6 text-xs font-mono text-botanical-green/60 whitespace-nowrap">{item.id}</td>
                    <td className="p-4 md:p-6 font-serif text-base md:text-lg text-botanical-green whitespace-nowrap">{item.name}</td>
                    <td className="p-4 md:p-6 text-xs text-botanical-green/60 whitespace-nowrap">{item.category}</td>
